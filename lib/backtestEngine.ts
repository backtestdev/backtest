...
export async function runBacktest(params: StrategyParameters): Promise<BacktestResult> {
  // Get stock database from FMP API (cached daily)
  ...
  const METRIC_LABELS: Record<string, string> = {
    ...
    profit_margin: "Profit margin", gross_margin: "Gross margin", operating_margin: "Operating margin",
    market_cap: "Market cap", price_to_book: "P/B", price_to_sales: "P/S", price_to_fcf: "P/FCF",
    debt_to_equity: "D/E", debt_to_assets: "D/A", current_ratio: "Current ratio",
    interest_coverage: "Interest coverage", roe: "ROE", roic: "ROIC",
    free_cash_flow_per_share: "FCF/share", payout_ratio: "Payout ratio",
    beta: "Beta", week52_high_pct: "52-wk high %", sector: "Sector",
    eps: "EPS", shares_change_pct: "Shares change %",
  };
  ...
  const appliedFilters = params.filters.map(f => {
    ...
    const fmtVal = (v: number) => {
      if (["dividend_yield", "revenue_growth", "earnings_growth", "profit_margin", "gross_margin", "operating_margin", "roe", "roic", "payout_ratio", "shares_change_pct", "debt_to_assets"].includes(f.metric)) {
